...
public class Cell3D {
	 ...
	 * Create a 3D cell
	 * 
	 * @param probe the data probe
	 */
	public Cell3D(DataProbe probe) {
		_probe = probe;

		//a vector for each corner
		for (int i = 0; i < 2; i++) {
			for (int j = 0; j < 2; j++) {
				for (int k = 0; k < 2; k++) {
	...
	/**
	 * Calculate the interpolated vector via trilinear interpolation
	 * 
	...
	public void calculate(double q1, double q2, double q3, float[] result) {
		...
		//if not contained, give the zero vector
