...
import org.jlab.detector.hits.ALERTDetHit;
import org.jlab.geom.prim.Point3D;
import org.jlab.geom.prim.Line3D;
import org.jlab.io.base.DataEvent;
import org.jlab.rec.banks.ALERTBaseHit;
import org.jlab.rec.banks.ALERTBaseHitReader;
import org.jlab.rec.banks.ALERTIMatchedHit;
...
public class ALERTHitReader implements ALERTIMatchedHit {

        public ALERTHitReader() {
            // TODO Auto-generated constructor stub
        }

        private List<ALERTHit> _ALERTHits;

        public List<ALERTHit> get_ALERTHits() {
            return _ALERTHits;
        }

        public void set_ALERTHits(List<ALERTHit> _ALERTHits) {
            this._ALERTHits = _ALERTHits;
        }
         ...
         */
        public void fetch_Hits(DataEvent event, long timeStamp, ALERTGeant4Factory geometry,
                               ArrayList<ALERTTrack> tracks,
        ...
        */
            ...
            ALERTBaseHitReader hitReader = new ALERTBaseHitReader();
            ALERTIMatchedHit MH = this;
            List<ALERTBaseHit> hitList = hitReader.get_MatchedHits(event, MH, triggerPhase, constants3, constants7);

            if (hitList.size() == 0) {
                // System.err.println("there is no FTOF bank ");
                _ALERTHits = new ArrayList<ALERTHit>();
                return;
            }

            // Instantiates the lists of hits
            List<ALERTHit> hits = new ArrayList<ALERTHit>();
            ...
            for (int i = 0; i < hitList.size(); i++) {
                id[i] = (i + 1);
                paddle[i] = hitList.get(i).get_Superlayer();
        ...
        private List<ALERTHit> matchHitsToCVTTrk(List<ALERTHit> ALERThits, ALERTGeant4Factory ALERTDetector, ArrayList<ALERTTrack> tracks) {
            if (tracks == null || tracks.size() == 0) {
                return ALERThits; // no hits were matched with DC tracks
            }
            // Instantiates the final list of hits
            List<ALERTHit> hitList = new ArrayList<ALERTHit>();

            // Instantiates map of track intersections with the paddles
            IndexedList<ArrayList<ALERTTrack>> trkHitsMap = new IndexedList<ArrayList<ALERTTrack>>(1);
            // calculate track intersections
            for (int i = 0; i < tracks.size(); i++) {
                ALERTTrack trk = tracks.get(i);
//            System.out.println(tracks.size() + " " + i + trk.toString());
                List<DetHit> trkHits = ALERTDetector.getIntersections(trk.getLine());
                if (trkHits != null && trkHits.size() > 0) {
                    for (DetHit hit : trkHits) {
                        ALERTDetHit trkHit = new ALERTDetHit(hit);
...
//                    System.out.println(trkHit.getPaddle() + " " + dir);
                        if(dir>0) {
                            ...
                            ALERTTrack ALERTTrkHit = new ALERTTrack(trk.getId(),trk.getLine(),trk.getPath());
                            ALERTTrkHit.setHit(trkHit);
                            // if map entry for the given paddle doesn't already exist, add it
                            if(!trkHitsMap.hasItem(trkHit.getPaddle())) {
                                ArrayList<ALERTTrack> list = new ArrayList<ALERTTrack>();
                                trkHitsMap.add(list, trkHit.getPaddle());
                            }
                            // add the track/intersection to the map
                            trkHitsMap.getItem(trkHit.getPaddle()).add(ALERTTrkHit);
                        }
                    }
                }
            }

            for(ALERTHit ALERTHit : ALERThits) {
...
//                    }
                ...
                for (int i = 0; i < hitlists.size(); i++) {
                    if (hitlists.get(i).size() > 0) {
                        ...
                        matchLists.add(hit);
                        if(debug>1) System.out.println(i+")  s "+hit.get_Sector()+" l "+hit.get_Layer()+" c "+hit.get_Superlayer()+" adcL "+hit.get_ADC1()+" adcR "+hit.get_ADC2()+" tdcL "+
